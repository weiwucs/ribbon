...
RibbonCtrlContainer::RibbonCtrlContainer(QWidget *containerWidget, QWidget *parent)
        : QWidget(parent), m_d(new RibbonCtrlContainerPrivate(this)) {
...
QSize RibbonCtrlContainer::sizeHint() const {
...
QSize RibbonCtrlContainer::minimumSizeHint() const {
    if (nullptr == m_d->containerWidget) {
...
QWidget *RibbonCtrlContainer::containerWidget() {
    return m_d->containerWidget;
}

const QWidget *RibbonCtrlContainer::containerWidget() const {
    return m_d->containerWidget;
}

void RibbonCtrlContainer::setEnableShowIcon(bool b) {
    m_d->enableDrawIcon = b;
    update();
}

void RibbonCtrlContainer::setEnableShowTitle(bool b) {
    m_d->enableDrawTitle = b;
    update();
}

void RibbonCtrlContainer::setContainerWidget(QWidget *w) {
...
void RibbonCtrlContainer::paintEvent(QPaintEvent *e) {
    Q_UNUSED(e);
    ...
    if (m_d->enableDrawIcon) {
        ...
        if (!icon.isNull()) {
            //绘制图标
            QSize iconSize = RibbonDrawHelper::iconActualSize(icon, &opt, QSize(widgetHeight, widgetHeight));
            RibbonDrawHelper::drawIcon(icon, &painter, &opt, x, 0, widgetHeight, widgetHeight);
            x += iconSize.width() + 4;
        }
    }
    //绘制文字
    if (m_d->enableDrawTitle) {
        ...
        if (!text.isEmpty()) {
            ...
            if (textWidth > 0) {
                RibbonDrawHelper::drawText(text, &painter, &opt, Qt::AlignLeft | Qt::AlignVCenter,
...
void RibbonCtrlContainer::resizeEvent(QResizeEvent *e) {
    Q_UNUSED(e);
    QStyleOption opt;
    initStyleOption(&opt);
    int x = 0;
...
void RibbonCtrlContainer::initStyleOption(QStyleOption *opt) {
    opt->initFrom(this);
}
